...
const signUp = async (req, res) => {
    ...
    if (user) {
        ...
        User.create(userData).then((data, err) => {
            ...
            else res.status(StatusCodes.CREATED).json({ message: 'User Created successfully' });
        })
    }
}
    ...
    try {
        ...
        if (user) {
            if (user.authenticate(req.body.password)) {
                ...
                res.status(StatusCodes.OK).json({ token, user: { _id, fistName, lastName, email, role, fullName } })
            } else {
